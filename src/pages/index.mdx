...
<Figure>
  <Image slot="figure" source={teleoperation_system} altText="teleoperation_system" class="w-full" />
  <span slot="caption">Bimanual robotic system comprising two Franka Research 3 arms equipped with ByteDexter V2 hands.</span>
</Figure>
...
<div class="text-justify">
We study long-horizon instruction following across three tasks of increasing difficulty. We first verify that a plain behavior-cloned VLA, trained solely on human teleoperated robot trajectories, can reliably execute long-horizon tasks. In the vacuum operation task, the robot first establishes a stable four‑finger grasp on a tabletop vacuum, powers it with a thumb press, increases power with a second thumb press, performs coverage sweeping to remove confetti, and then powers it off. Serving bread requires using tongs to retrieve a croissant from a pastry container while  maintaining grasping stability, simultaneous object stabilization, and precise, compliant manipulation. 
</div>
